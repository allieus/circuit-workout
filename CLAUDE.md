...
- **타이머 화면 표시** (`TimerView.jsx`): 상단 스텝 점(휴식 중엔 다음 동작을 **빈 점 + "다음 동작 n/N" 라벨**로 예고 — 진행과 구분) + 진행 바 아래 전체 서킷 라인업(완료 취소선·현재 강조, 세로 640px 미만에선 숨김). 휴식 화면의 큰 동작명은 다음 동작 예고임(기획.md "휴식=예습 시간") — 같은 동작이 연속 반복되는 버그 아님. 랜덤 서킷은 5슬롯=5패턴 각 1개라 인접 중복이 구조적으로 불가능(10초/50초 프리셋만 의도적 반복). body 배경을 페이즈 색으로 동기화해 넓은 화면에서도 여백 없이 채워진다.
