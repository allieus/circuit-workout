...
- **프로그램 프리셋** (`defaults.js` PRESETS + `App.jsx` applyPreset): 케틀벨-7가지.md의 프로그램(10초/50초·무빙 타겟·딥 식스) + **어린이 코스**를 고정 서킷 + 권장 설정으로 적용. 어린이 전용 동작(KIDS_EXERCISES, id c1~c4)은 서고·랜덤 풀에 넣지 않는다 — 어른 서킷에 섞이지 않게 프리셋에서만 참조(applyPreset의 find 체인: 서고 → DEFAULT → KIDS). 음성·삽화 파이프라인에는 포함. 별도 타이머 로직 없음 — 기존 상태 머신 그대로(10초/50초는 1동작 서킷이라 rest 없이 work→roundRest 반복). 적용 후 설정 조정 자유. "다시 뽑기"(랜덤 생성)로 해제되고, 프리셋 중에는 홈의 개별 ↻ 숨김. 라운드 상한 30 — 라운드 음성 클립도 30까지 사전 생성돼 있음. 서고에서 지운 기본 동작도 프리셋에선 DEFAULT_EXERCISES 폴백으로 동작.
