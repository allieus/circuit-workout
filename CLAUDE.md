...
- **대상 + 장비** (`App.jsx` pickRandom/changeAudience/toggleGear): 두 축뿐이다.
  - **대상** `settings.audience ∈ adult | kids` — kids는 소스 자체가 전용 풀(KIDS_EXERCISES)로 바뀐다(어른 서고와 완전 분리). kids 진입 시 KIDS_SETTINGS(2라운드·20초/20초) 자동 적용, 나갈 때 복원은 없음.
  - **장비** `settings.gear = { kb, db, pullup, dipbar, rope, kdb }` — 주력 도구(kb·db·어린이 아령 kdb)는 디폴트 on, 보조 장비(철봉·딥바·줄넘기)는 off. **맨몸 동작은 항상 후보, 장비 동작은 그 칩이 켜져 있을 때만 후보**(`requiredGear`가 equip kb/db와 gear 태그를 한 규칙으로 통일). 다 켜면 예전 "전체", 다 끄면 "맨몸" 모드와 동등. 꺼진 장비의 동작은 폴백에서도 절대 안 나온다. 대상/장비 변경 시 뽑아둔 서킷 다시 뽑기(프리셋 중엔 유지). equip 없는 구버전 사용자 추가 동작은 항상 후보.
  - **마이그레이션**: 구버전 `settings.mode`(all|kb|db|body|kids)는 로드 시 대상+장비 조합으로 1회 변환 후 제거(App.jsx).
