...
src/
  ...
  data/defaults.js         # PATTERNS(5패턴+색), MODES(운동 모드), GEAR(보유 장비 옵션), DEFAULT_EXERCISES(기본 51동작+equip/gear — 패턴×모드 조합마다 2개 이상 유지), DEFAULT_SETTINGS, artUrl/ytUrl
...
- **운동 모드** (`App.jsx` pickRandom): `settings.mode ∈ all | kb | db | body | kids` — kb/db/body는 동작의 `equip` 태그로 서고 풀을 필터, **kids는 소스 자체가 전용 풀(KIDS_EXERCISES)로 바뀐다**(어른 서고와 완전 분리 — 전체 모드에 어린이 동작이 섞이지 않고, 어린이 모드에 어른 동작이 안 나옴). kids 진입 시 KIDS_SETTINGS(2라운드·20초/20초) 자동 적용, 나갈 때 복원은 없음. 모드에 맞는 동작이 패턴에 없으면 모드 무시하고 패턴 전체에서 뽑는 폴백. 모드 변경 시 뽑아둔 서킷이 있으면 즉시 다시 뽑는다(프리셋 중엔 유지). equip 없는 동작(구버전 사용자 추가분)은 "전체"에서만 등장.
- **보유 장비 옵션** (`defaults.js` GEAR + `App.jsx` toggleGear): `settings.gear = { pullup, dipbar, rope }`(디폴트 전부 off) — 모드와 별개 축. `gear` 태그가 달린 동작(풀업·행잉 니 레이즈·딥스·딥바 니 레이즈·줄넘기)은 해당 토글이 켜져 있어야 뽑기 후보가 되며, **pickRandom의 모든 폴백 단계에서도 gear 필터는 유지**(장비 없는 사용자에게 장비 동작이 절대 안 나옴). 토글 변경 시 뽑아둔 서킷 다시 뽑기(프리셋 중엔 유지). 홈의 "보유 장비" 칩 UI.
- **프로그램 프리셋** (`defaults.js` PRESETS + `App.jsx` applyPreset): 케틀벨-7가지.md의 프로그램(10초/50초·무빙 타겟·딥 식스)을 고정 서킷 + 권장 설정으로 적용. 어린이는 프리셋이 아니라 **모드**다(위 운동 모드 참조) — 전용 동작(KIDS_EXERCISES, id c1~c10 + 슈퍼맨·점핑잭 재사용)은 음성·삽화 파이프라인에 포함(generate-voice.mjs가 KIDS까지 순회). 별도 타이머 로직 없음 — 기존 상태 머신 그대로(10초/50초는 1동작 서킷이라 rest 없이 work→roundRest 반복). 적용 후 설정 조정 자유. "다시 뽑기"(랜덤 생성)로 해제되고, 프리셋 중에는 홈의 개별 ↻ 숨김. 라운드 상한 30 — 라운드 음성 클립도 30까지 사전 생성돼 있음. 서고에서 지운 기본 동작도 프리셋에선 DEFAULT_EXERCISES 폴백으로 동작.
