    ...
    <div className="app">
      ...
      <div className="add-card">
        ...
        <div className="contact-desc">
          궁금한 점, 불편한 점, 추가됐으면 하는 동작·기능을 자유롭게 남겨주세요.
        </div>
        <textarea
        ...
        <input
          ...
          placeholder="website"
        />
        <button className="btn add-submit" disabled={!message.trim() || status === "sending"} onClick={submit}>
          {status === "sending" ? "보내는 중..." : "보내기"}
        </button>
        {status === "done" && <div className="contact-result contact-result--ok">전송됐어요. 확인 후 연락드릴게요!</div>}
        {status === "error" && (
          <div className="contact-result contact-result--err">전송에 실패했어요 — 네트워크 확인 후 다시 시도해 주세요.</div>
        )}
      </div>
    </div>
  );
